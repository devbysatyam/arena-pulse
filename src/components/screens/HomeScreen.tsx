...
import { MATCH, SECTIONS, AMENITIES } from '@/lib/stadium-data';
...
export default function HomeScreen() {
  ...
  const queueColor = (q: string) =>
    q === 'low' ? '#00ff9d' : q === 'moderate' ? '#ffb800' : '#ff4d6a';
  const queueBg = (q: string) =>
    q === 'low' ? 'rgba(0,255,157,0.15)' : q === 'moderate' ? 'rgba(255,184,0,0.15)' : 'rgba(255,77,106,0.15)';
    ...
    <div className="min-h-dvh bg-[#0a0b14] overflow-y-auto pb-28">
      ...
      <header className="flex items-center justify-between px-5 pt-14 pb-3">
        <div>
          ...
          <h1 className="font-headline font-black text-xl text-white">
            {user?.name?.split(' ')[0] ?? 'Fan'} 👋
          </h1>
        </div>
        <button
          onClick={() => navigate('profile')}
          className="w-10 h-10 rounded-full overflow-hidden border-2 border-purple-500/30"
          aria-label={`View profile for ${user?.name ?? 'Fan'}`}
        >
          {user?.avatar ? (
            <img
              src={user.avatar}
              alt={`${user.name ?? 'Fan'} profile photo`}
            ...
            <div
              ...
              aria-hidden="true"
            >
              <span className="text-white font-bold text-sm">{user?.name?.[0] ?? 'F'}</span>
            </div>
          )}
        </button>
      </header>
      ...
      <section
        ...
        style={{
          background: 'linear-gradient(135deg,rgba(124,95,240,0.2),rgba(0,212,255,0.1))',
      ...
      >
        <div className="px-5 py-4">
          ...
          {/* Score is a live region so screen readers announce changes */}
          <div
            role="status"
            aria-live="polite"
            aria-label={`Chelsea FC vs Arsenal FC, score ${MATCH.score}, minute ${MATCH.minute}`}
            className="flex items-center justify-between"
          >
            <div className="text-center flex-1">
              <p className="text-xs text-white/50 mb-1">Chelsea FC</p>
              <div
                className="w-12 h-12 rounded-full bg-blue-600/20 border border-blue-500/30 mx-auto flex items-center justify-center text-2xl"
                aria-hidden="true"
              >
                🔵
              </div>
            ...
            <div className="text-center flex-1">
              ...
              <div
                className="w-12 h-12 rounded-full bg-red-600/20 border border-red-500/30 mx-auto flex items-center justify-center text-2xl"
                aria-hidden="true"
              >
                🔴
              </div>
      ...
      {/* Stats row */}
      <section aria-label="Stadium statistics" className="px-5 mb-5">
      ...
      {/* Quick Actions */}
      <section aria-label="Quick actions" className="px-5 mb-5">
        <p
      ...
      <div
        role="alert"
        aria-label="Crowd density warning for South Stand"
      ...
      >
        ...
        <div className="flex-1">
          ...
          <p className="text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>
            Avoid Gate E until density drops. Use Gate A instead.
          </p>
        </div>
        <button
          onClick={() => navigate('heatmap')}
          className="text-xs font-bold px-3 py-1.5 rounded-xl"
          style={{ color: '#ff4d6a', background: 'rgba(255,77,106,0.15)' }}
          aria-label="View crowd heatmap"
        >
          View Map
        </button>
      </div>
      ...
      {/* Nearby Food */}
      <section aria-label="Nearest food stalls" className="px-5 mb-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            style={{ color: '#00d4ff' }}
            aria-label="See all food stalls"
          >
            See all →
          </button>
        </div>
        <div className="space-y-2.5">
          {nearbyFood.map(f => (
            <button
              ...
              style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}
              aria-label={`${f.label} — ${f.waitMin} minute wait, ${f.queue} queue`}
            >
              <div
              ...
              <div className="flex-1 text-left">
                ...
                <p className="text-xs" style={{ color: 'rgba(255,255,255,0.45)' }}>
                  {f.level === 1 ? 'Level 1' : 'Ground'} · {f.waitMin}min wait
                </p>
              </div>
              <div
                className="flex items-center gap-1 px-2.5 py-1 rounded-full"
                style={{ background: queueBg(f.queue ?? 'low') }}
                aria-hidden="true"
              >
                <div
                  className="w-1.5 h-1.5 rounded-full"
                  style={{ background: queueColor(f.queue ?? 'low') }}
                />
                <span
                  className="text-[10px] font-bold capitalize"
                  style={{ color: queueColor(f.queue ?? 'low') }}
                >
                  {f.queue}
                </span>
              </div>
      ...
      {/* AI Suggestion */}
      <div
        className="mx-5 mb-4 rounded-2xl px-4 py-3 flex items-center gap-3 cursor-pointer"
        style={{
          background: 'linear-gradient(135deg,rgba(124,95,240,0.12),rgba(0,212,255,0.06))',
          border: '1px solid rgba(124,95,240,0.25)',
        }}
        onClick={aiSheetOpen}
        onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') aiSheetOpen(); }}
        role="button"
        tabIndex={0}
        aria-label="Open Arena AI assistant — West concourse is clearest right now"
      >
        <div
        ...
        <div className="flex-1">
          ...
          <p className="text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>
            West concourse is clearest right now. Best time to visit Stall G1! 🍔
          </p>
        </div>
        <span className="material-symbols-outlined text-white/30 text-sm" aria-hidden="true">
          chevron_right
        </span>
      </div>
    </div>
